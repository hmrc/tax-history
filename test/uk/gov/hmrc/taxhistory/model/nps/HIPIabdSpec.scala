...
import play.api.libs.json.{JsError, JsNull, JsObject, JsResultException, JsValue, Json, JsonValidationError}
...
class HIPIabdSpec extends TestUtil with AnyWordSpecLike with Matchers with OptionValues {
  ...
  "HIPIabd Json" should {
    "transform Iabds Response Json correctly to Employment Model " in {
      val iabd = Json.parse(iabdJsonResponse).as[HIPIabd]
  ...
  "HIPIabd" when {
    "toStatePension is called" should {
      val testIabd = Json.parse(iabdJsonResponse).as[HIPIabd]
