...
class HipConnectorSpec extends BaseConnectorSpec {
  ...
  lazy val testIabd: List[Iabd]                      = loadFile("/json/nps/response/iabds.json").as[IabdList].getListOfIabd
  ...
  ".getCorrelationId" when {
    ...
    "requestID is present in the headerCarrier" should {
      "return new ID pre-appending the requestID when the requestID matches the format(8-4-4-4)" in {
        ...
        hipNpsConnectorWithUUID.getHIPCorrelationId(HeaderCarrier(requestId = Some(RequestId(requestId)))) mustBe
      ...
      "return new ID when the requestID does not match the format(8-4-4-4)" in {
        ...
        hipNpsConnectorWithUUID.getHIPCorrelationId(HeaderCarrier(requestId = Some(RequestId(requestId)))) mustBe uuid
      }
    }
    ...
    "requestID is not present in the headerCarrier should return a new ID" should {
      "return the uuid" in {
        ...
        hipNpsConnectorWithUUID.getHIPCorrelationId(HeaderCarrier()) mustBe uuid
      }
    }
  }

  "create the correct hip headers" in {
    val headers = hipNpsConnectorWithUUID.buildHeaders(using hc)
    headers mustBe List(
