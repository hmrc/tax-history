...
class CompanyBenefitControllerSpec extends PlaySpec with OneServerPerSuite with MockitoSugar with TestUtil with BeforeAndAfterEach  {
  ...
  object TestCompanyBenefitController extends CompanyBenefitController{
  ...
  "getBenefits" must {
    ...
    "respond with OK for successful get" in {
      ...
      when(mockEmploymentHistoryService.getBenefits(Matchers.any(), Matchers.any(), Matchers.any())(Matchers.any[HeaderCarrier]))
        .thenReturn(Future.successful(HttpResponse(OK, Some(successResponseJson))))
      val result = TestCompanyBenefitController.getBenefits(nino, 2016, employmentId).apply(FakeRequest())
