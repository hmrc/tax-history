...
class DesConnector @Inject()(val http: HttpGet,
  ...
  def getIabds(nino: Nino, year: Int): Future[List[Iabd]] = {
    ...
    val timerContext = metrics.startTimer(MetricsEnum.DES_GET_IABDS)

    val result = http.GET[List[Iabd]](iabdsUrl(nino, year))
    result.onSuccess { case _ =>
      timerContext.stop()
      metrics.incrementSuccessCounter(MetricsEnum.DES_GET_IABDS)
    }
    result.onFailure { case e =>
      metrics.incrementFailedCounter(MetricsEnum.DES_GET_IABDS)
      timerContext.stop()
      logger.warn(s"DES connector - Error returned from DES connector (getIabds): ${e.toString}: ${e.getMessage}")
    }
    result
  }

  def getTaxAccount(nino: Nino, year: Int): Future[DesTaxAccount] = {
    implicit val hc = basicDesHeaders(HeaderCarrier())

    val timerContext = metrics.startTimer(MetricsEnum.DES_GET_TAX_ACCOUNT)

    val result = http.GET[DesTaxAccount](taxAccountUrl(nino, year))
    result.onSuccess { case _ =>
      timerContext.stop()
      metrics.incrementSuccessCounter(MetricsEnum.DES_GET_TAX_ACCOUNT)
    }
    result.onFailure { case e =>
      metrics.incrementFailedCounter(MetricsEnum.DES_GET_TAX_ACCOUNT)
      timerContext.stop()
      logger.warn(s"DES connector - Error returned from DES connector (getTaxAccount): ${e.toString}: ${e.getMessage}")
    }
    result
  }
}