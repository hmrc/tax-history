...
class EmploymentHistoryService @Inject() (
  ...
  @tailrec
  private def getFillers(
    employments: List[Employment],
    employmentGapFillers: List[Employment],
    taxYear: TaxYear
  ): List[Employment] =
    if (employments.nonEmpty) {
      val dateAlignedFillers =
        employmentGapFillers flatMap (filler => alignFillerDates(filler, employments.head, taxYear))
      getFillers(employments.tail, dateAlignedFillers, taxYear)
    } else {
      employmentGapFillers
    }

  def getEmployment(nino: Nino, taxYear: TaxYear, employmentId: String)(implicit
    ...
    if (taxYear == TaxYear.current) {
      ...
      getFromCache(nino, taxYear).flatMap { paye =>
        ...
        if (paye.allowances.isEmpty) {
          Future.failed(
            new NotFoundException(s"Allowance not found for NINO ${nino.value} and tax year ${taxYear.toString}")
          )
        } else {
  ...
  def getPayAndTax(nino: Nino, taxYear: TaxYear, employmentId: String)(implicit
    headerCarrier: HeaderCarrier
  ): Future[Option[PayAndTax]] =
    getFromCache(nino, taxYear)
      .map(_.payAndTax.get(employmentId))
      .orNotFound(
        s"PayAndTax not found for NINO ${nino.value}, tax year ${taxYear.toString} and employmentId $employmentId"
      )

  def getAllPayAndTax(nino: Nino, taxYear: TaxYear)(implicit
    headerCarrier: HeaderCarrier
  ): Future[Map[String, PayAndTax]] =
    getFromCache(nino, taxYear)
      .map(_.payAndTax)
    ...
    if (taxYear == TaxYear.current.previous) {
      getFromCache(nino, taxYear).flatMap { paye =>
        ...
        paye.taxAccount match {
          ...
          case None             =>
            Future.failed(
              new NotFoundException(s"TaxAccount not found for NINO ${nino.value} and tax year ${taxYear.toString}")
            )
        }
      }
    } else {
  ...
  def getCompanyBenefits(nino: Nino, taxYear: TaxYear, employmentId: String)(implicit
    headerCarrier: HeaderCarrier
    ...
    if (taxYear == TaxYear.current) {
      ...
      getFromCache(nino, taxYear)
        .map(_.benefits.getOrElse(employmentId, List.empty))
        .orNotFound(
          s"CompanyBenefits not found for NINO ${nino.value}, tax year ${taxYear.toString} and employmentId $employmentId"
        )
    }
  ...
  def retrieveAndBuildPaye(nino: Nino, taxYear: TaxYear)(implicit
    ...
    } yield mergeEmployments(nino, taxYear, npsEmployments, rtiEmployments, taxAccountOpt, iabds)
    paye
  }
  ...
  def mergeEmployments(
    ...
    val employmentMatches: Map[NpsEmployment, RtiEmployment] =
      EmploymentMatchingHelper.matchEmployments(npsEmployments, rtiEmployments)
    ...
    EmploymentHistoryServiceHelper
      .combinePAYEs(payes)
      .copy(
  ...
  def retrieveNpsEmployments(nino: Nino, taxYear: TaxYear): Future[List[NpsEmployment]] = {
    ...
    desNpsConnector
      .getEmployments(nino, passedInTaxYear)
      .map { employments =>
  ...
  private def auditNpsEventHelper(nino: String, nps: NpsEmployment): Map[String, String] =
    Map(
      "nino"              -> nino,
      "payeNumber"        -> nps.payeNumber,
      "taxDistrictNumber" -> nps.taxDistrictNumber,
      "worksNumber"       -> nps.worksNumber.getOrElse("")
    )

  private def sendDataEvents(records: List[DataEventDetail], auditType: DataEventAuditType)(implicit
    hc: HeaderCarrier
  ): Unit =
    if (records.nonEmpty) {
      auditable.sendDataEvents(transactionName = PAYEForAgents, details = records, eventType = auditType)
    }
  ...
  private def filterStatePension(iabds: List[Iabd]): (List[Iabd], Option[Iabd]) = {
    ...
    (iabds.filterNot(matchStatePension), iabds.find(matchStatePension))
  }
}
