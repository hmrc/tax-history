...
object NpsEmploymentWithoutNino {
  ...
  given reader: Reads[NpsEmploymentWithoutNino]  = (js: JsValue) => {
    ...
    for {
      ...
      startDate                    <- (js \ "startDate").validateOpt[LocalDate](using JsonUtils.npsDateFormat)
      endDate                      <- (js \ "endDate").validateOpt[LocalDate](using JsonUtils.npsDateFormat)
  ...
  given writer: Writes[NpsEmploymentWithoutNino] = Json.writes[NpsEmploymentWithoutNino]
}
